...
<div style="width:80%" class="container bg-dark p-5 m-5  shadow ">
    <form action="#" method="POST" enctype="multipart/form-data">
        <div class="d-flex justify-content-between p-3 mb-5 border-bottom">
            <h1 class="text-white">Edit Card <span style="font-size:5rem; font-weight:100; margin-right:-13px;  position:relative; top:20px;">/</span><span style="font-size:5rem;font-weight:100; position:relative; color:aqua; top:20px;">/</span> <span style="color:aqua; "> ADMIN</span></h1>
            <a href="managecard.php" type="button" class="btn  text-white btn-primary h-75 mt-2 text-center">Manage
                Cards </a>
        </div>
        <?php
...
if (isset($_GET['id'])) {
    ...
    $data1 = mysqli_fetch_assoc($result1);
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (
                $name != "" && $image != "" ) { 
                $query = "UPDATE cards SET name='$name',image='$image' WHERE id='$id'";
        ...
        <div class="mb-3 d-flex flex-column">
            <label for="exampleFormControlInput1" class="form-label text-white"><strong>Card Name :</strong></label>
            <input type="text" class="form-control mb-2" id="exampleFormControlInput1"  value=<?=$data1['name'];?> name="name">
        </div>
        <div class="mb-3 d-flex flex-column">
            <label for="exampleFormControlInput1" class="form-label text-white"><strong>Card Image :</strong></label>
            <div class="d-flex">
                <input type="text" class="form-control h-50 w-75 me-1 mt-1" id="imgbox"  value=<?=$data1['image'];?>  readonly name="image">
